...
html_path = "HTML/"
Equities_path = os.path.join(html_path, "*Forex*.htm*")
#<span data-name="USD/CNH" data-id="961728" class="alertBellGrayPlus js-plus-icon genToolTip oneliner" data-tooltip="Create Alert"></span>

Equities_pattern = r'<span\sdata-name="([^><"]+)"\sdata-id="(\d+)"'
...
for file_path in dirs:
    ...
    for symbol_match in symbols_match:
        ...
        if int(curr_id_str) < 1:
           continue
        symbol_str = "Currencies_" + symbol_match.group(1).replace('/','_').replace('?','').replace('=','')
        candles_filename = os.path.join('Output/Currencies/' + symbol_str + '.txt')
